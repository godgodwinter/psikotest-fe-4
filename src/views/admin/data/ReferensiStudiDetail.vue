...
storeAdminBar.setPagesActive("referensistudi");
...
const getData = async () => {
  try {
    const response = await Api.get(`siswa/data/referensi/${id}`);
  ...
  <div class="pt-4 px-10 md:flex justify-between" v-if="data">
    <div>
      <span
        ...
        >Detail Referensi Studi & Kerja</span
      >
    </div>
    <div class="md:py-0 py-4">
      <BreadCrumb>
        <template v-slot:content>
          Klasifikasi Referensi Studi <BreadCrumbSpace /> Index
        </template>
      </BreadCrumb>
    </div>
  </div>
      ...
      <div class="overflow-x-auto px-4 space-y-10 py-2">
        <div class="space-y-2">
          ...
          <div class="space-y-2 border-b-2 py-2">
            <p class="indent-8 text-gray-700">{{ data.nama }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white shadow rounded-lg px-4 py-2">
      <div class="overflow-x-auto px-4 space-y-10 py-2">
        <div class="space-y-2">
          ...
          <div class="space-y-2 border-b-2 py-2">
            <p class="indent-8 text-gray-700">{{ data.jenis }}</p>
          </div>
        </div>
      </div>
    </div>
